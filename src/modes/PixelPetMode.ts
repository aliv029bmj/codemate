...
export class PixelPetMode extends BaseMode {
  ...
  private checkLevelUp(): void {
    ...
    if (this.experience >= experienceNeeded) {
      ...
      // Notify the user
      vscode.window.showInformationMessage(`Your Code566 pet reached level ${this.level}!`);
    }
  }
  ...
  private loadState(context: vscode.ExtensionContext): void {
    this.mood = context.globalState.get('code566.pixelpet.mood', 'neutral') as 'happy' | 'neutral' | 'sad';
    this.energy = context.globalState.get('code566.pixelpet.energy', 100);
    this.experience = context.globalState.get('code566.pixelpet.experience', 0);
    this.level = context.globalState.get('code566.pixelpet.level', 1);
    this.lastUpdateTime = context.globalState.get('code566.pixelpet.lastUpdateTime', Date.now());
  ...
  private saveState(context: vscode.ExtensionContext): void {
    context.globalState.update('code566.pixelpet.mood', this.mood);
    context.globalState.update('code566.pixelpet.energy', this.energy);
    context.globalState.update('code566.pixelpet.experience', this.experience);
    context.globalState.update('code566.pixelpet.level', this.level);
    context.globalState.update('code566.pixelpet.lastUpdateTime', Date.now());
  }
